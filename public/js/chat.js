const chatForm = document.getElementById('msg-input');      //id du bouton pour envoyer message
const chatMessages = document.querySelector('.messages')    //Va contenir les nouveaux messages
const nomC = document.getElementById('nomClasse')           //Va contenir le nom de la classe a l<acceuil
const ul = document.getElementById('userList')     

//Recuperer le nom d'utilisateur et la classe a partir du url
...
//Envoyer le le nom d'utilisateur et la classe recuperés
// du Url au serveur
socket.emit("joindreEleve", {utilisateur, classe});
...
//Message prof du serveur
socket.on('messageProf', message =>{
    afficheMessageProf(message);
    //Quand on recoit un nouveau message, aller a ce message
    chatMessages.scrollTop = chatMessages.scrollHeight;    
});
...
//Afficher un message automatique ou provenant d'un eleve dans le chat
function afficheMessage(message){
    ...
    li.setAttribute("class", "sent");                   //Attribuer une classe à l'élément li (pour le css)

    ul.appendChild(li);                                 //Ajouter l'élément li à la liste générale ul
}

//Afficher le message provenant du prof dans le chat
function afficheMessageProf(message){
...
//Afficher la liste des autres eleves connectés
